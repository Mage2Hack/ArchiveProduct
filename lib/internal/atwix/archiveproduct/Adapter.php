    ...
    public function toggleArchiveProducts($productIds, $isArchived = 1, $storeId = 0)
    {
        if (!is_array($productIds) || empty($productIds)) {
            ...
            try {
                ...
                if (NULL != $isArchived) {
                    $this->_addMessage(sprintf('A total of %1 record(s) have been moved to trash.',
                        count($productIds)), 'success');
                } else {
                    $this->_addMessage(sprintf('A total of %1 record(s) have been restored.',
                        count($productIds)), 'success');
                }
            } catch (\Exception $e) {
    ...
    public function destroyProducts($productIds)
    {
        if (!is_array($productIds) || empty($productIds)) {
            ...
            try {
                ...
                $this->_addMessage(sprintf('A total of %1 record(s) have been deleted.',
                    count($productIds)), 'success');
            } catch (\Exception $e) {
                $this->_addMessage($e->getMessage(), 'error');
            }
